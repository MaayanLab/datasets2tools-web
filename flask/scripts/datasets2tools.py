...
r.source('scripts/datasets2tools.R')
pandas2ri.activate()
...
def fetch_archs4(series_platform):

    # Create dataset
    dataset = Dataset()

    # Get matrix url
    matrix_url = 'https://s3.amazonaws.com/mssm-seq-series-platform/{series_platform}_series_matrix.txt.gz'.format(**locals())
    ...
    # Get platform
    platform_accession = os.path.basename(matrix_url).split('_')[1]
...
def fetch_gtex(tissue='all', data_type='gene'):

    # Get label
    label = data_type if tissue == 'all' else '_'.join([data_type, tissue])

    # Get RData URL
    rdata_url = 'http://duffel.rail.bio/recount/SRP012682/rse_{label}.Rdata'.format(**locals())

    # Get GTEx data
    gtex_data = r.fetch_gtex(rdata_url)

    # Create dataset
    dataset = Dataset()

    # Add expression data
    dataset.rawcount_dataframe = pandas2ri.ri2py(gtex_data[0])

    # Add sample metadata
    dataset.sample_metadata_dataframe = pandas2ri.ri2py(gtex_data[1])
...
def fetch_dataset(dataset_id, dataset_source):

    if dataset_source == 'archs4':
        dataset = fetch_archs4(dataset_id)
    elif dataset_source == 'gtex':
        pass
