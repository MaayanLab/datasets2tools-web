...
##############################
    ...
    def make_extension_toolbar(self, analysis_summary_dataframe, dataset_accession):
        ...
        toolbar_html = 
            "<div class='d2t-wrapper' id='{dataset_accession}'><div class='d2t-toolbar'><div class='d2t-logo'><img class='d2t-icon' src='http://labs.icahn.mssm.edu/maayanlab/wp-content/uploads/sites/75/2014/10/D2T-150x150.png'></div>".format(**locals()) + 
            "".join(["<div class='tool-icon-wrapper'><img class='tool-icon' src='{tool_icon_url}' data-tool-name='{tool_name}'><div class='d2t-tooltip'><div class='tool-name'>{tool_name}</div><div class='tool-count'>{count}</div><div class='tool-description'>{tool_description}</div></div></div>".format(**tool_annotation_dict) for tool_annotation_dict in tool_annotation_list]) + 
            "</div></div>"
        return toolbar_html
...
##############################
    ...
    def make_extension_tool_table(self, analysis_summary_dataframe, dataset_accession):
        ...
        tool_table_html = 
            "<div class='d2t-wrapper' id='{dataset_accession}'><table class='d2t-tool-table'><tr><th class='tool-header'>Tool</th><th class='description-header'>Description</th><th class='canned-analyses-header'>Canned Analysis</th></tr>" +
            "".join(["<tr><td class='tool-cell'><a href='{tool_homepage_url}'><img class='tool-icon' src='{tool_icon_url}'><div class='tool-name'>{tool_name}</div></a></td><td class='description-cell'>{tool_description}</td><td class='canned-analyses-cell'><i class='fa fa-plus-square fa-1x' aria-hidden='true'></i></td></tr>".format(**tool_annotation_dict) for tool_annotation_dict in tool_annotation_list]) +
            "</table></div>"
...
##############################
    ...
    def make_extension_canned_analysis_table_dict(self, analysis_summary_dataframe, page_type, page_size=5):
        ...
        row_dict = {tool_name:["<tr><td class='link-cell'><a href='{canned_analysis_url}'><img class='tool-icon' src='{tool_icon_url}'></a></td><td class='title-cell'><div class='canned-analysis-title'>{canned_analysis_title}<div class='d2t-tooltip'>{canned_analysis_description}</div></span></td><td class='metadata-cell'><i class='fa fa-info-circle fa-1x view-metadata' aria-hidden='true'></i><div class='d2t-tooltip'><ul>".format(**canned_analysis)+"".join(["<li><span class='metadata-tag'>"+key.replace('_', ' ').title()+"</span>: "+value+"</li>" for key, value in canned_analysis['metadata'].iteritems()])+"</ul></div><i class='fa fa-download fa-1x download-metadata' aria-hidden='true'></i><div class='d2t-tooltip'>JSON<br>TXT</div></td><td class='share-cell'><i class='fa fa-share-alt fa-1x share' aria-hidden='true'></i><div class='d2t-tooltip'>{canned_analysis_url}</div></td></tr>".format(**canned_analysis) for canned_analysis in canned_analysis_dict.values()] for tool_name, canned_analysis_dict in analysis_dict.iteritems()}
        table_dict = {tool_name: ["<table class='canned-analysis-table'><tr><th class='link-header'>Link</th><th class='title-header'>Title</th><th class='metadata-header'>Metadata</th><th class='share-header'>Share</th></tr>"+"".join(x)+"</table>" for x in chunks(row_list, page_size)] for tool_name, row_list in row_dict.iteritems()}
        ...
        if page_type == 'search':
            table_dict = {tool_name: ["<div class='tool-annotation'><div class='go-back'><i class='fa fa-backward fa-2x' aria-hidden='true'></i></div><img class='tool-icon' src='{tool_icon_url}'><a href='{tool_homepage_url}' class='tool-name'>{tool_name}</a><div class='tool-description'>{tool_description}</div></div>".format(**tool_annotation_dict[tool_name])+row for row in row_list] for tool_name, row_list in table_dict.iteritems()}
        elif page_type == 'landing':
            table_dict = {tool_name: ["<div class='tool-annotation'><div class='go-back'><<< Go Back</div><img class='search-tool-icon' src='{tool_icon_url}'><div class='tool-name'>{tool_name}</div><div class='tool-description'>{tool_description}<br>The table below allows to browse.</div></div>".format(**tool_annotation_dict[tool_name])+row for row in row_list] for tool_name, row_list in table_dict.iteritems()}
        return table_dict
