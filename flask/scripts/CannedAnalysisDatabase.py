...
##############################
    ...
    def make_extension_toolbar(self, analysis_summary_dataframe, dataset_accession):
        ...
        toolbar_html = "<div class='d2t-wrapper' id='{dataset_accession}'><div class='d2t-toolbar'><div class='d2t-logo'><img class='d2t-logo-img' src='http://labs.icahn.mssm.edu/maayanlab/wp-content/uploads/sites/75/2014/10/D2T-150x150.png'></div>".format(**locals()) + "".join(["<div class='tool-icon-wrapper d2t-tooltip'><img class='tool-icon-img' src='{tool_icon_url}' data-tool-name='{tool_name}'><div class='d2t-tooltip-text d2t-tooltip-top tool-info-tooltip'><div class='tool-icon-tooltip-name'>{tool_name}</div><div class='tool-icon-tooltip-count'>{count}</div><div class='tool-icon-tooltip-description'>{tool_description}</div></div></div>".format(**tool_annotation_dict) for tool_annotation_dict in tool_annotation_list]) + "</div></div>"
        return toolbar_html
...
##############################
    ...
    def make_extension_canned_analysis_table_dict(self, analysis_summary_dataframe, page_type, page_size=5):
        ...
        if page_type == 'search':
            table_dict = {tool_name: ["<div class='search-tool-annotation'><div class='reset-toolbar'><i class='fa fa-backward fa-2x' aria-hidden='true'></i></div><img class='search-tool-annotation-img' src='{tool_icon_url}'><div class='search-tool-annotation-text'><a href='{tool_homepage_url}' class='search-tool-annotation-name'>{tool_name}</div><div class='search-tool-annotation-description'>{tool_description}</div></div></div>".format(**tool_annotation_dict[tool_name])+row for row in row_list] for tool_name, row_list in table_dict.iteritems()}
