...
##############################
    ...
    def make_extension_toolbar(self, analysis_summary_dataframe, dataset_accession):
        ...
        toolbar_html = "<div class='d2t-wrapper' id='{dataset_accession}'><div class='d2t-toolbar'><div class='d2t-logo'><img class='d2t-logo-img' src='http://labs.icahn.mssm.edu/maayanlab/wp-content/uploads/sites/75/2014/10/D2T-150x150.png'></div>".format(**locals()) + "".join(["<div class='tool-icon-wrapper d2t-tooltip'><img class='tool-icon-img' src='{tool_icon_url}' data-tool-name='{tool_name}'><div class='d2t-tooltip-text tool-info-tooltip'><div class='tool-icon-tooltip-name'>{tool_name}</div><div class='tool-icon-tooltip-count'>{count}</div><div class='tool-icon-tooltip-description'>{tool_description}</div></div></div>".format(**tool_annotation_dict) for tool_annotation_dict in tool_annotation_list]) + "</div></div>"
        return toolbar_html
