...
##############################
    ...
    def make_extension_toolbar(self, analysis_summary_dataframe, dataset_accession):
        ...
        toolbar_html = "<div class='d2t-wrapper' id='{dataset_accession}'><div class='d2t-toolbar'>".format(**locals()) + "".join(["<div class='d2t-tooltip'><img class='tool-icon-img' src='{tool_icon_url}' data-tool-name='{tool_name}'><div class='d2t-tooltip-text tool-info-tooltip'><p class='tool-icon-tooltip-name'>{tool_name}</p><p class='tool-icon-tooltip-count'>{count}</p><p class='tool-icon-tooltip-description'>{tool_description}</p></div></div>".format(**tool_annotation_dict) for tool_annotation_dict in tool_annotation_list]) + "</div></div>"
        return toolbar_html
