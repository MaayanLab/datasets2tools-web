...
##############################
    ...
    def make_extension_toolbar(self, analysis_summary_dataframe, dataset_accession):
        ...
        toolbar_html = "<div class='d2t-wrapper' id='{dataset_accession}'><div class='d2t-toolbar'>".format(**locals()) + "".join(["<div class='tool-icon-wrapper d2t-tooltip'><img class='tool-icon-img' src='{tool_icon_url}' data-tool-name='{tool_name}'><div class='d2t-tooltip-text tool-info-tooltip'><p class='tool-icon-tooltip-name'>{tool_name}</p><p class='tool-icon-tooltip-count'>{count}</p><p class='tool-icon-tooltip-description'>{tool_description}</p></div></div>".format(**tool_annotation_dict) for tool_annotation_dict in tool_annotation_list]) + "</div></div>"
        return toolbar_html
...
##############################
    ...
    def make_extension_canned_analysis_table_dict(self, analysis_summary_dataframe, page_type, page_size=5):
        ...
        row_dict = {tool_name:["<tr><td class='link-cell'><a href='{canned_analysis_url}'><img class='canned-analysis-icon-img' src='{tool_icon_url}'></a></td><td class='title-cell'>{canned_analysis_title}<sup><i class='fa fa-info-circle fa-1x d2t-tooltip' aria-hidden='true'><div class='d2t-tooltip-text description-tooltip'>{canned_analysis_description}</div></i></sup></td><td class='metadata-cell'><i class='fa fa-info-circle fa-1x d2t-tooltip' aria-hidden='true'><div class='d2t-tooltip-text view-metadata-tooltip'><ul>".format(**canned_analysis)+"".join(["<li><span class='metadata-tag'>"+key.replace('_', ' ').title()+"</span>: "+value+"</li>" for key, value in canned_analysis['metadata'].iteritems()])+"</ul></div></i><i class='fa fa-download fa-1x d2t-tooltip' aria-hidden='true'><div class='d2t-tooltip-text download-metadata-tooltip'>JSON<br>TXT</div></i></td><td class='share-cell'><i class='fa fa-share-alt fa-1x d2t-tooltip' aria-hidden='true'><div class='d2t-tooltip-text'>{canned_analysis_url}</div></i></td></tr>".format(**canned_analysis) for canned_analysis in canned_analysis_dict.values()] for tool_name, canned_analysis_dict in analysis_dict.iteritems()}
        ...
        if page_type == 'search':
            table_dict = {tool_name: ["<div class='search-tool-annotation'><div class='reset-toolbar'><i class='fa fa-backward fa-2x' aria-hidden='true'></i></div><img class='search-tool-annotation-img' src='{tool_icon_url}'><div class='search-tool-annotation-text'><div class='search-tool-annotation-name'>{tool_name}</div><div class='search-tool-annotation-description'>{tool_description}</div></div></div>".format(**tool_annotation_dict[tool_name])+row for row in row_list] for tool_name, row_list in table_dict.iteritems()}
