...
fetch_gtex <- function(rdata_url) {

	# Load library
	require(SummarizedExperiment)

	# Load data
	rdata <- load(url(rdata_url))

	# Get expression dataframe
	expression_dataframe <- as.data.frame(assay(get(rdata), 1))

	# Set gene symbol rownames
	# rownames(expression_dataframe) <- as.data.frame(rowData(rse_gene)$symbol)$value

	# Get annotation
	sample_annotation_dataframe <- as.data.frame(colData(get(rdata), 1))[,c('sampid', 'smrin', 'smts', 'smtsd', 'smtstptref')]

	# Get list
	result_list <- list(expression_dataframe=expression_dataframe, sample_annotation_dataframe=sample_annotation_dataframe)
...
run_limma <- function(rawcount_dataframe, design_dataframe) {
	# Load packages
	require(limma)
	require(edgeR)
	...
	# Get results
	limma_dataframe <- topTable(fit2, adjust="BH", number=nrow(rawcount_dataframe))
...
run_characteristic_direction <- function(rawcount_dataframe, design_dataframe, constant_threshold) {
